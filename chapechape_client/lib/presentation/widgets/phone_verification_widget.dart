...
class _PhoneVerificationWidgetState extends State<PhoneVerificationWidget> {
  ...
  void _startTimer() {
    ...
    if (_expiresAt != null) {
      ...
      _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (mounted) { // Vérifier si le widget est encore monté
          setState(() {
          ...
          // Si le widget n'est plus monté, arrêter le timer
          _timer?.cancel();
        }
      });
    }
  ...
  Future<void> _requestCode() async {
    ...
    if (_selectedPhoneNumber == null || _selectedPhoneNumber!.phoneNumber.isEmpty) {
                ...
                Column(
                  ...
                  children: [
                    TextFormField(
                      ...
                      enabled: !_isLoading, // Désactiver seulement pendant le loading
                      autofocus: true, // Focus automatique pour faciliter la saisie
                      ...
                      maxLength: 6,
                      onChanged: (value) {
                        // Auto-vérifier quand on a 6 chiffres
